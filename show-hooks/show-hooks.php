...
class ABC_Show_Hooks {
	...
	*/
	...
	public function enqueue_script($screen) {
		// Main Styles
		wp_register_style( 'abc-main-css', plugins_url('/block-injector-with-wooCommerce/show-hooks/assets/css/abc-main.css', basename( __FILE__ ) ), '', '1.1.0', 'screen' );
		wp_enqueue_style( 'abc-main-css' );
		// Main Scripts
		wp_register_script( 'abc-main-js', plugins_url( '/block-injector-with-wooCommerce/show-hooks/assets/js/abc-main.js', basename( __FILE__ ) ), array('jquery'), '1.1.0' );
	...
	public function hook_all_hooks( $hook ) {
		...
		$hooks = explode("_",$hook);
		if( $hooks[0] == 'woocommerce' ){
